    ...
    if ($user_id > 0) {
        ...
        if ($rating >= 1 && $rating <= 5) {
            try {
                ...
                printError("Error submitting your review: " . $e->getMessage());
            }
        } else {
...
try {
    ...
    // Fetch bulk discounts for this product
...
<style>
    ...
    .stars-inner::before { content: '★★★★★'; color: #fbbf24; /* Yellow-400 */ }
...
<main class="container mx-auto py-8 px-4 md:px-8">
    <?php if ($product): ?>
            ...
            <div class="md:col-span-7">
                <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
                    <?php if(!empty($product['category'])): ?>
                        <p class="text-xs text-gray-500 uppercase tracking-wide mb-1"><?= htmlspecialchars($product['category']) ?></p>
                    <?php endif; ?>
                    
                    <h1 class="text-2xl md:text-3xl font-bold text-gray-900 mb-2 leading-tight"><?= htmlspecialchars($product['name']) ?></h1>
                    ...
                    <form id="add-to-cart-form">
                        ...
                        <?php if (!empty($options)): ?>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                                <?php foreach ($options as $option_name => $option_values): ?>
                                    <div>
                                        <label for="option-<?= strtolower($option_name) ?>" class="block text-sm font-medium text-gray-700 mb-1"><?= htmlspecialchars($option_name) ?></label>
                                        <select id="option-<?= strtolower($option_name) ?>" name="options[<?= htmlspecialchars($option_name) ?>]" class="block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md border">
                        ...
                        <div class="flex items-center gap-4 mt-4">
                            ...
                            <button type="submit" id="add-to-cart-btn" class="btn btn-primary flex-grow h-[38px]">
                                Add to Cart
                            </button>
                        </div>
        ...
        <div class="mt-12 max-w-4xl">
            ...
            <?php if ($user_id > 0 && !$user_has_reviewed): ?>
                <div class="mb-8 p-5 border rounded-lg bg-gray-50">
                    <h3 class="text-sm font-bold text-gray-700 mb-3">Write a Review</h3>
                    <form action="product-detail.php?id=<?= $product_id ?>" method="POST">
                        <input type="hidden" name="action" value="submit_review">
                        <div class="mb-3">
                            <label class="block text-xs font-medium text-gray-500 mb-1">Rating</label>
                            <div class="rating-form">
                                <div class="rating-stars justify-start flex-row">
                        ...
                        <div class="mb-3">
                            <label for="review_text" class="block text-xs font-medium text-gray-500 mb-1">Review</label>
                            <textarea id="review_text" name="review_text" rows="3" class="w-full p-2 border border-gray-300 rounded-md text-sm" placeholder="Share your thoughts..."></textarea>
                        </div>
                        <button type="submit" class="btn btn-primary text-xs py-2">Submit</button>
                    </form>
                </div>
            <?php endif; ?>
                ...
                <?php else: ?>
                    <?php foreach ($reviews as $review): ?>
                        <div class="pb-4 border-b border-gray-100">
                            ...
                            <p class="text-sm text-gray-700"><?= nl2br(htmlspecialchars($review['review_text'])) ?></p>
                        </div>
                    <?php endforeach; ?>
                <?php endif; ?>
            </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <?php foreach ($related_products as $related_product): ?>
                        <a href="product-detail.php?id=<?= $related_product['id'] ?>" class="block bg-white border border-gray-100 rounded-lg overflow-hidden hover:shadow-md transition-all group">
                            ...
                            <div class="p-3">
                                <h3 class="font-medium text-gray-900 text-sm truncate mb-1"><?= htmlspecialchars($related_product['name']) ?></h3>
                                <?php 
...
<script>
// Store product data from PHP for use in JavaScript
const productData = {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updatePriceDisplay() {
        ...
        if (isNaN(quantity) || quantity < productData.minQuantity) {
            quantity = productData.minQuantity;
        ...
        for (const tier of productData.bulkDiscounts) {
            if (quantity >= tier.min_quantity) {
                applicableDiscount = parseFloat(tier.discount_percentage);
            }
        }
        ...
        } else if (productData.salePrice !== null) {
            discountInfo.textContent = 'On Sale!';
        } else {
            discountInfo.textContent = '';
        }
        
        // Update the main price display to show price per item
        ...
        if(applicableDiscount > 0) {
             priceToShow = priceToShow * (1 - (applicableDiscount / 100));
        }
    ...
    cartForm.addEventListener('submit', function(e) {
        ...
        if (!allOptionsSelected) {
            showCartMessage('Please select all product options.', 'error');
            return;
        }
        
        // All good, submit via fetch
        ...
        .then(resData => {
            if (resData.success) {
                ...
                if (document.getElementById('cart-count')) {
                     ...
                     badge.textContent = resData.cart_count;
