...
// --- FETCH CONTENT ---
$hero_title = htmlspecialchars(EnvLoader::get('HERO_TITLE', 'Welcome to the Mall'));
$hero_subtitle = htmlspecialchars(EnvLoader::get('HERO_SUBTITLE', 'Explore independent stores and unique finds.'));
            ...
            <div class="mb-16">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <?php foreach ($featured_stores as $store): 
                        ...
                        // Fix for null store name
                        $sName = $store['store_name'] ?? 'Store';
                    ?>
                        <a href="store.php?vendor_id=<?= $store['id'] ?>" class="group block bg-white rounded-lg shadow-sm hover:shadow-md border border-gray-100 overflow-hidden transition">
                            <div class="p-6 flex items-center space-x-4">
                                <img src="<?= htmlspecialchars($logo) ?>" alt="<?= htmlspecialchars($sName) ?>" class="w-16 h-16 rounded-full object-cover border group-hover:scale-105 transition">
                                <div>
                                    <h3 class="font-bold text-gray-800 group-hover:text-blue-600 transition"><?= htmlspecialchars($sName) ?></h3>
            ...
            <div class="mb-16">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                    <?php foreach ($recent_products as $product): 
                        ...
                        // FIX: Ensure store_name is not null using '??'
                        $vendorName = $product['store_name'] ?? 'Asiko Mall'; 
                        // FIX: Ensure image is not null
                    ...
                    ?>
                        <article class="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-md transition border border-gray-100 flex flex-col">
                            <a href="product-detail.php?id=<?= $product['id'] ?>" class="block relative h-48 overflow-hidden bg-gray-100">
                                ...
                                <span class="absolute bottom-0 left-0 bg-black bg-opacity-60 text-white text-xs px-2 py-1">
                                    Sold by: <?= htmlspecialchars($vendorName) ?>
                                </span>
                            </a>
                            <div class="p-4 flex-1 flex flex-col">
                                <p class="text-xs text-gray-500 mb-1"><?= htmlspecialchars($product['category'] ?? 'General') ?></p>
        ...
        <?php if (!empty($categories)): ?>
            ...
            <div class="categories-grid grid grid-cols-2 md:grid-cols-4 gap-4">
                <?php foreach ($categories as $category): 
                    ...
                    // FIX: Ensure category name is safe
                    $catName = $category['category'] ?? 'Uncategorized';
                ?>
                    <a href="products.php?category=<?= urlencode($catName) ?>" class="group block relative rounded-lg overflow-hidden h-32 md:h-40">
                        ...
                        <div class="absolute inset-0 bg-black bg-opacity-40 flex flex-col justify-center items-center text-white p-2 text-center">
                            <h3 class="font-bold text-lg"><?= htmlspecialchars($catName) ?></h3>
